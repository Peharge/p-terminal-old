...
// Encrypt plaintext using DPAPI (Local Machine scope)
...
BOOL StorePasswordSecure(const char* password) {
    BYTE* blob     = NULL;
    DWORD blobSize = 0;
    if (!EncryptData(password, &blob, &blobSize))
        return FALSE;
...
// Load and decrypt password from Credential Manager
BOOL LoadPasswordSecure(char** outPwd) {
    PCREDENTIALW pcred = NULL;
    if (!CredReadW(CRED_TARGET_NAME, CRED_TYPE_GENERIC, 0, &pcred))
        return FALSE;

    BOOL result = FALSE;
    if (pcred->CredentialBlob && pcred->CredentialBlobSize > 0) {
        result = DecryptData(pcred->CredentialBlob,
...
wchar_t* BuildWSLCommand(const wchar_t* base, const wchar_t* args) {
    size_t len = wcslen(L"wsl.exe ") + wcslen(base)
               + (args && args[0] ? wcslen(args) + 1 : 0) + 1;
    wchar_t* cmd = (wchar_t*)malloc(len * sizeof(wchar_t));
    if (!cmd) return NULL;
    wcscpy(cmd, L"wsl.exe ");
    wcscat(cmd, base);
    if (args && args[0]) {
        wcscat(cmd, L" ");
        wcscat(cmd, args);
...
void ShowMenu() {
    ...
    wprintf(L"2) View History\n");
    wprintf(L"3) Change Password\n");
    wprintf(L"4) Clear Password\n");
    wprintf(L"5) Exit\n");
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    while (running) {
        ...
        // consume newline
        fgetws(argv[0], 1, stdin);

        switch (choice) {
            case 1: {
                wchar_t base[256] = {0}, args[512] = {0};
                ...
                if (wcsncmp(base, L"sudo", 4) == 0 && !password) {
                    ...
                    buf[strcspn(buf, "\n")] = '\0';
                    if (StorePasswordSecure(buf)) {
                        password = _strdup(buf);
