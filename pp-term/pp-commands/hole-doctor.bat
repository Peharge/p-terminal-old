...
set "security_check=C:\Users\%USERNAME%\p-terminal\pp-term\pp-commands\install-pp-term.py"

if not exist "%security_check%" (
    echo Error: Python script not found: %security_check%
    exit /B 1
)

REM Run the Python script in the background
start "" "%PYTHON_PATH%" "%security_check%"

REM Wait for 5 seconds before continuing
timeout /t 5 /nobreak

set "security_check=C:\Users\%USERNAME%\p-terminal\pp-term\security\run-security-check.py"
...
set "doctor=C:\Users\%USERNAME%\p-terminal\pp-term\pp-commands\doctor.py"
